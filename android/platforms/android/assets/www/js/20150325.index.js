    ...
    sort: function () {        
        ...
        this.index.sort(		
			function (a, b) {				
				var tka = a.split(' ');
				var tkb = b.split(' ');				
				tka[0] = tka[0].replace(/\,/g, '.');
				tkb[0] = tkb[0].replace(/\,/g, '.');
				if (!isNaN(tka[0]) && !isNaN(tkb[0])) {				
					var n = (100 * parseFloat(tka[0]));
					n -= (100 * parseFloat(tkb[0]));					
					return n;
				}
				else {
					var n = 0;
...
{
	if (s == null) return '';
	//if (dataset.isPA || dataset.joinTable == 'TR041')	
	if (dataset.joinTable == 'TR041')	
	{
		s = s.toUpperCase();	
...
{		
	var html = '';    
	var formaSelezionata  = decodificaFormaFarmaceutica(dataset.formaSelezionata).toUpperCase();
	
	$("#resultsHeader").html('');	
    $("#resultsList").empty();
	
	html += '<ul data-role="listview" data-autodividers="false" data-inset="true">';			
	html += '<li data-role="list-divider" style="color: #808080; height: 40px;">';
	html += '<span class="x-scrolling">';
	html += 'ELENCO DEI FARMACI EQUIVALENTI<br/>' + 
			dataset.ricercaIniziale + ' - ' + 
			formaSelezionata + ' ' + 
			dataset.dosaggioSelezionato;
	html += '</span>';
	html += '</li>';
    ...
    for (var i = 0; data != null && i < data.length; i++) {
        ...
        if (rimborso != ND) {
			...
			if (rec.classe == 'C') {
				html += '<br/><i>Differenza a carico del cittadino</i>';
			}
			else if (rec.classe == 'A') {				
				html += '<br/><i>Differenza a carico del cittadino 0 &euro;</i>';					
			}			
...
function OnClickForma(forma, dosaggi)
{
	...
	if (selectionMap_ != null) {
		...
		for (var i = 0; i < d.dosaggi.length; i ++) {			
			...
			var _d = descriviDosaggio(d.dosaggi[i].dosaggio);	
			
            if (map.contains(_d)) continue;            
			
			map.put(_d, d.dosaggi[i].dosaggio);       			
		}		
		...
		for (var i = 0; i < map.length(); i ++) {			
			...
			html += '<a href="javascript: OnClickDosaggio(\'' + forma + '\', \'' + map.getSortedValue(i) + '\')" ';
			...
			html += '<span class="ui-btn-text">' + map.getSortedKey(i) + '</span>';
			html += '</span>';
			html += '</a>'										
			html += '</td></tr>';			
		}		
...
{
	dataset.formaSelezionata = forma;
	dataset.dosaggioSelezionato = descriviDosaggio(dosaggio);
		...
		switch (dataset.joinTable) {
			...
			case "APP_CA":			
				...
				_sql = fillcommand (
						...
						(dataset.isPA ?
							...
							:
							[									
								'text:' + getInputValue() + "%" + dosaggio.trim() + "%"
							]
						)
				);
				break;
			...
			case "TR041":			
				...
				for (var i = 0; i < lista_aic.length; i ++) {
					if (lista_aic[i] == '') continue;
					if (icount > 0) sql_append += ',';
					icount ++;
					sql_append += '\'';
			...
			case "TR041":			
				_sql = fillcommand(
						"select distinct top 1000 TR041.FDI_T507 as codeGR, 'GR' as classe " +			
							...
							[
								'text:' + getInputValue() + "*" + forma + "*" + dosaggio 
							]
						);
				break;
	...
	$.ajax ({
			...
			success: function (data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if (!aic && data[i].aic) aic = data[i].aic;
				...
				if (productsFilter.length == 0) {
					if (!dataset.isPA) {
						productsFilter = "APP_CA.FDI_4875 like '" + getInputValue() + "%'";
					}
					else {						
						productsFilter = "APP_CA.FDI_0001 = '" + aic + "'";						
...
function descriviDosaggio1(x)
{
    var s = x.toUpperCase();
	for (var i = 0; i < forma_f_.length; i += 2) {
		if (s.indexOf(forma_f_[i]) >= 0) {
			var j = s.indexOf(forma_f_[i]);
			var n = forma_f_[i].length;
			j += n;
			var p = x.substring(j).trim().split(' ');
			var ret = '';
			for (var k = 0; k < p.length; k ++) {
				if (ret.length == 0 && isNaN(p[k])) continue;
				ret += ' ';
				ret += p[k];
			}
			return ret;
...
function descriviDosaggio2(x)
{
	var ret = '';
	var p = x.split(' ');
	for (var k = p.length - 1; k >= 0; k --) {
		if (!isNaN(p[k]) && k == p.length - 1) continue;
		ret = p[k] + ' ' + ret;
		if (!isNaN(p[k])) {
			break;
		}
...
var descriviDosaggio = descriviDosaggio3;
	...
	$.ajax ({
		...
		data: { 			
			sql: fillcommand (
				(dataset.joinTable == 'TR041' ?
					...
					"	where FDI_4875 like ? " +
					"-- and not (FDI_1010 is null and FDI_1094 is null) " +
					") " + 
					"order by 1 asc"
                    :
					"select distinct upper(substring(APP_CA.FDI_4875, charindex('*', APP_CA.FDI_4875) + 1, 1000)) as dx, APP_CA.FDI_0371 as forma " + 
					"from APP_CA " +
					"where " + 
		...
		success: function (data) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				var fp = (dataset.joinTable == 'TR041' ?
					data[i].dx.split('*')
					: [ data[i].forma, data[i].dx ]);
			...
			for (var j = 0; j < selectionMap_.length; j ++) {				
				...
				html += '<span class="ui-btn-inner" style="vertical-align: middle;">';
				html += '<span class="ui-btn-text" style="font-size: 14px;">';
				html += decodificaFormaFarmaceutica (selectionMap_[j].forma);
				html += '</span>';
				html += '</span>';
				html += '</a>';										
				html += '</td></tr>';												
		...
		data: { 
			sql: fillcommand (
				(dataset.joinTable == 'TR041' ? 
					...
					"order by 1 asc"),
				[
					'text:' + v + (dataset.joinTable == 'TR041' ? '*%' : '')
				]
			), 
			encoding: dataset.encoding
		},                     
