...
function cercaprodottiPerNome () {
	...
	$('#resultsHeader').html(
		'Elenco dei farmaci equivalenti per<br/>' + value + '<br/>' + forma_ + '&nbsp;' + dosaggio_);
				
	cercaprodotti (_sql); 
		
}

function cercaprodottiPerAIC (value) {
...
function selezionaDosaggio(dosaggio)
{	
	dosaggio_ = dosaggio;	
	
	//
	// TODO generalizzare espressione forma_ (e.g.: 20CPR -> %CPR%)
	//
	...
	$.ajax ({
			...
			success: function (data) {									
				...
				if (!data || data.length <= 0) {					
					msgbox ('Non esistono famaci equivalenti<br/>al prodotto selezionato: ' + getInputValue(), 'Avviso', 200);
					return;
				}
				...
				for (var i = 0; i < data.length; i++) {
					...
					case 'H':						
						if (productsFilter.length > 0) productsFilter += " OR ";
						productsFilter += "FDI_1010 = '" + data[i].codeA + "'";
						break;
					
					case 'C':
					case 'CN':
						if (productsFilter.length > 0) productsFilter += " OR ";
						productsFilter += "FDI_1094 = '" + data[i].codeC + "'";
						break;
...
function deviceReadyInitializer()
{
		...
		$.ajax ({
			...
			success: function (data) {						
				...
				for (var i = 0; i < data.length; i++) {
					...
					if (listaForme_.indexOf('|' + fp[0] + '|') < 0) {							
					
						html += '<tr><td align="left">';					
						html += '<a href="javascript: selezionaForma(\'' + fp[0] + '\')" '						
						...
						html += '<span class="ui-btn-text">' + descriviForma (fp[0]) + '</span>';
						html += '</span>';
						html += '</a>'										
						html += '</td></tr>';
					...
					if (listaDosaggi_.indexOf('|' + fp[1] + '|') < 0) {					
					
						savedHtml += '<tr><td align="left">';					
						savedHtml += '<a href="javascript: selezionaDosaggio(\'' + fp[1] + '\', \'P\')" ';							
						...
						savedHtml += '<span class="ui-btn-text">' + descriviDosaggio(fp[1]) + '</span>';
						savedHtml += '</span>';
						savedHtml += '</a>'										
						savedHtml += '</td></tr>';
    ...
    $("#autocomplete").on("filterablebeforefilter", function (e, data) {
        ...
        if (value && value.length > 2) {
			...
			var _sql = fillcommand (
				"select distinct top 1000 " +
				"  left(TS052.FDI_T504, charindex('-', TS052.FDI_T504) - 1) " +
				"from TS052 " +
				"where " +
				"  TS052.FDI_T504 like ? " +
				"order by 1 asc",
            ...
            .then (function (response) {
				$.each(response, function (i, val) {										
					html += "<li><a href=\"javascript:onAutocompleteClick('" + 
							val + "', 'P')\"><span style=\"color: #707070;\">" + val + "</span></a></li>";
                });				
                $ul.html(html);
                $ul.listview("refresh");
