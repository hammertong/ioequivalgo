...
namespace webapp
{
    ...
    class MessageBuffer
    {
        ...
        int buffer_ptr = 0;
        string[] buffer = new string[MAX_BUF];

        public void append(params object[] args)
        {
            lock (buffer)
            {
                buffer[buffer_ptr++] = "{" + String.Format(pattern, args) + "}";
                buffer_ptr %= MAX_BUF;
            }
        }
        ...
        public override string ToString()
        {
            ...
            lock (buffer)
            {
                while (count++ < MAX_BUF)
                {
                    ...
                    if (line == null) break;
                    b.Append(b.Length > 1 ? ", " : "").Append(line);
                    i--;
                    if (i < 0) i = MAX_BUF - 1;
                }
            }
    ...
    {
        static MessageBuffer buffer = new MessageBuffer();
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                if (request.QueryString["stats"] != null)
                {
